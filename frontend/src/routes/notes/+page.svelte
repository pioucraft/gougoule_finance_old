<div id="wrapper">
    <div id="leftBar">
        ...
        {#each currentFilesAndFolders as file}
            ...
            {#if file.endsWith(".html")}
                <button on:click={async () => [openedNoteContent, opened] = await openNote(url, file)} id="leftBar-filesAndFolders-{file}" class="leftBar-item">{file.split(".html")[0].split("/")[file.split("/").length-1]}</button>
            {/if}
        {/each}
    ...
    <div id="editor">
        ...
        <div id="editor-title">
            <h1 id="editor-title-title">{opened.split("/")[opened.split("/").length-1].split(".html")[0]}</h1>
            {#if opened}
