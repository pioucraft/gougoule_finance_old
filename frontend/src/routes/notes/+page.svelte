        ...
        <div id="editor-infos">
            ...
            {#if opened.endsWith(".md")}
                ...
                {:else}
                    <p>Not saved</p>
                {/if}
                <button on:click={() => document.getElementById("filePicker").click()} id="editor-infos-uploadImage"><svg data-slot="icon" aria-hidden="true" fill="none" stroke-width="1.5" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M12 16.5V9.75m0 0 3 3m-3-3-3 3M6.75 19.5a4.5 4.5 0 0 1-1.41-8.775 5.25 5.25 0 0 1 10.233-2.33 3 3 0 0 1 3.758 3.848A3.752 3.752 0 0 1 18 19.5H6.75Z" stroke-linecap="round" stroke-linejoin="round"></path></svg></button>
...
<input on:change={async () => openedNoteContent = await uploadImage(url, openedNoteContent)} id="filePicker" type="file" style="visibility: hidden; position:fixed;">
...
<script>
    ...
    import { createNewFolder, createNewNote, expandFolder, makeData, editNote, deleteNote, openNote, textAreaResize, saveNote, editOrNoteANote, uploadImage } from "./script"
